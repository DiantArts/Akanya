...
engine::graphic::actor::Positions::iterator Positions::begin()
{
    if (auto* position { std::get_if<SinglePosition>(&m_variant) }) {
        return engine::graphic::actor::Positions::iterator(position);
    } else {
        auto& positions { std::get<MultiplePositions>(m_variant) };
        return engine::graphic::actor::Positions::iterator(&positions[0]);
    }
}

engine::graphic::actor::Positions::const_iterator Positions::begin() const
{
    if (const auto* position { std::get_if<SinglePosition>(&m_variant) }) {
        return engine::graphic::actor::Positions::const_iterator(position);
    } else {
        auto& positions { std::get<MultiplePositions>(m_variant) };
        return engine::graphic::actor::Positions::const_iterator(&positions[0]);
    }
}

engine::graphic::actor::Positions::const_iterator Positions::cbegin() const
{
    if (const auto* position { std::get_if<SinglePosition>(&m_variant) }) {
        return engine::graphic::actor::Positions::const_iterator(position);
    } else {
...
{
    if (auto* position { std::get_if<SinglePosition>(&m_variant) }) {
        return engine::graphic::actor::Positions::iterator(position + 1);
    } else {
        auto& positions { std::get<MultiplePositions>(m_variant) };
        return engine::graphic::actor::Positions::iterator(&positions[positions.size()]);
    }
}

engine::graphic::actor::Positions::const_iterator Positions::end() const
{
    if (const auto* position { std::get_if<SinglePosition>(&m_variant) }) {
        return engine::graphic::actor::Positions::const_iterator(position + 1);
    } else {
        auto& positions { std::get<MultiplePositions>(m_variant) };
        return engine::graphic::actor::Positions::const_iterator(&positions[positions.size()]);
    }
}

engine::graphic::actor::Positions::const_iterator Positions::cend() const
{
    if (const auto* position { std::get_if<SinglePosition>(&m_variant) }) {
        return engine::graphic::actor::Positions::const_iterator(position + 1);
    } else {
